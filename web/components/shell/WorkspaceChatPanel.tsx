...
export default function WorkspaceChatPanel() {
  ...
  const send = useCallback(async () => {
    ...
    try {
      ...
      if (!res.ok || !res.body) {
        setMessages(prev => [...prev, { role: 'assistant', content: 'Sorry, something went wrong. The AI service may be temporarily unavailable.' }])
        return
      }
  ...
  return (
    ...
    <div className="fixed bottom-0 left-1/2 -translate-x-1/2 z-50 flex flex-col items-center gap-0 pb-4 backdrop-blur-md" style={{ width: 'min(680px, calc(100vw - 48px))' }}>
          ...
          >
            ...
            <div className="flex items-center justify-between px-4 py-2.5 border-b border-slate-100">
              <div className="flex items-center gap-2">
                <Bot className="w-3.5 h-3.5 text-[#4B83C4]" />
                <span className="text-xs font-semibold text-slate-700 tracking-tight">Workspace AI</span>
                <span className="text-[10px] text-slate-300 bg-slate-100 px-1.5 py-0.5 rounded font-medium">{wsId}</span>
              </div>
              <div className="flex items-center gap-1">
                {messages.length > 0 && (
                  <button
                    type="button"
                    onClick={() => setMessages([])}
                    className="text-[10px] text-slate-400 hover:text-slate-700 px-2 py-0.5 rounded hover:bg-slate-100 transition-colors cursor-pointer"
                  >
                    Clear
                  </button>
                )}
            ...
            <div className="overflow-y-auto px-4 py-3 space-y-3 scrollbar-minimal" style={{ maxHeight: 300 }}>
              {messages.length === 0 && (
                <div className="flex flex-col items-center gap-2 py-6 text-center">
                  ...
                  <p className="text-xs text-slate-400">Ask anything about your captured notes</p>
                  {['What did I save about React hooks?', 'Summarize my recent captures', 'What domains do I visit most?'].map(s => (
                    <button key={s} type="button"
                      onClick={() => { setInput(s); setTimeout(() => inputRef.current?.focus(), 50) }}
                      className="text-[11px] text-[#4B83C4] hover:underline cursor-pointer"
                    >{s}</button>
                  ))}
                </div>
              )}
      ...
      <div className={cn(
        'w-full flex items-center gap-2 px-4 py-2.5 rounded-2xl border border-slate-200 bg-white/80 backdrop-blur-md transition-all duration-150',
        open && 'rounded-t-none border-t-0',
      )}>
        <Sparkles className="w-4 h-4 text-[#4B83C4] shrink-0" />
        <input
          ref={inputRef}
          value={input}
          ...
          className="flex-1 text-sm bg-transparent border-none outline-none placeholder:text-slate-300 text-slate-700"
          disabled={loading}
        />
        <div className="flex items-center gap-1 shrink-0">
          {open && input.trim() && (
            <button
              ...
              onClick={() => void send()}
              disabled={loading}
              className="w-7 h-7 flex items-center justify-center rounded-lg bg-[#191919] text-white disabled:opacity-40 hover:bg-[#150C00] transition-colors cursor-pointer"
            >
          ...
          {!open && (
            <button
              ...
              className="text-[11px] text-slate-300 hover:text-slate-700 transition-colors cursor-pointer px-1"
            >
              ⌘K
            </button>
          )}
